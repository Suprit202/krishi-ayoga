...
const AlertFeed = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      // Fallback to static alerts if API fails
  ...
  const getFallbackAlerts = () => {
    // This would be your fallback data if API fails
    return [
      { 
        type: 'warning', 
        message: 'Ivermectin withdrawal period ends tomorrow', 
        timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
        priority: 'high'
      },
      { 
        type: 'info', 
        message: 'New drug added: Penicillin G', 
        timestamp: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
        priority: 'medium'
      },
      { 
        type: 'success', 
        message: 'All livestock groups are healthy', 
